...
void sound_PB6_T2 (const void *argu)
{
	while(1)
	{
		PB6_on();
		osDelay(cn*2);		//delay in ms
		PB6_off();
		osDelay(cn*2);		//delay in ms
	}
}

osThreadId T1, T2;			//Define thread handles
osThreadDef(sound_PB6_T1, osPriorityNormal, 1, 0);
osThreadDef(sound_PB6_T2, osPriorityNormal, 1, 0);
...
int main (void)
{
	...
	T2 = osThreadCreate(osThread(sound_PB6_T2), NULL);		//Create Thread
